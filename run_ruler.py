...
def main(args):
    ...
    for i in tqdm(range(0, len(prompt_list), args.eval_batch_size)):
        ...
        if args.method != "FullKV":
            if args.method.lower() in ["snapkv","pyramidkv","h2o","layerquant"]:
                window_sizes = 8
            ...
            for i in range(layers):
                ...
                model.model.layers[i].self_attn.config.begin_layer=args.quant_begin_layers
                model.model.layers[i].self_attn.config.end_layer=args.quant_end_layers
                model.model.layers[i].self_attn.config.quant_layers_bits=args.quant_layers_bits
        ...
        if args.quant_method == None:        
            ...
            output = model.generate(
                **tokenized_prompts,
                ...
                cache_config={"nbits": args.nbits, "backend": "HQQ","device":"cuda","residual_length":output_max_len,"axis_key":args.key_axis,"q_group_size":args.group_size},
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--quant_method",type=str,default=None,choices=["kivi","kvquant","layerquant"])
    parser.add_argument("--nbits", type=int, default=8, help="")
    parser.add_argument("--max_capacity_prompts", type=int, default=512, help="")
    parser.add_argument("--max_capacity_prompts_ratio", type=float, default=-1, help="")
    parser.add_argument("--steps", type=int, default=-1, help="maximum number of examples to evaluate per task.")
    parser.add_argument("--key_axis", type=int, default=1, help="key axis for quantization,1 as default kivi, 0 for flexgen")
    parser.add_argument("--quant_layers_bits",type=int, default=0, help="number of bits for quantized layers in layerquant")
    parser.add_argument("--quant_begin_layers",type=int,default=0)
    parser.add_argument("--quant_end_layers",type=int,default=0)
    ...
    elif args.quant_method == "layerquant":
        from pyramidkv.quantcache import LayerHQQQuantizedCache
        from transformers import cache_utils
        cache_utils.HQQQuantizedCache = LayerHQQQuantizedCache
