...
def mistral_flash_attn2_forward_LayerQuant(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    key_states = repeat_kv(key_states, self.num_key_value_groups)
    value_states = repeat_kv(value_states, self.num_key_value_groups)
    if self.layer_idx>=self.config.begin_layer and self.layer_idx<self.config.end_layer:
        nbits=self.config.quant_layers_bits
        size=16/nbits
        quant=False
        if nbits==16:
            quant=True
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len: 
            self.kv_seq_len = kv_seq_len 
            self.kv_cluster.reset_capacity(size)
            key_states_compress, value_states_compress = self.kv_cluster.update_kv(key_states, query_states, value_states, attention_mask, self.num_key_value_groups)
            past_key_value.update(key_states_compress, value_states_compress, self.layer_idx, nbits,quant,cache_kwargs)
        else:
            self.kv_seq_len += q_len
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, nbits,quant,cache_kwargs)
        past_key_value._seen_tokens=self.kv_seq_len
